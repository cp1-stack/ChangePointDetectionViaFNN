...
logger1 = logging.getLogger('outputs')
logger1.setLevel(logging.DEBUG)

fileHandler = logging.FileHandler(filename='test_acc.log')
...
train_dataset = CPDatasets('ChangePoints_data_train_beta10.csv')
train_loader = DataLoader(dataset=train_dataset, batch_size=1, shuffle=True, num_workers=2)

test_dataset = CPDatasets('ChangePoints_data_test_beta10.csv')
...
if __name__ == '__main__':
    ...
    for epoch in range(Epoch):
        ...
        # logger1.info('---------第{}轮训练开始---------'.format(epoch + 1))
        # logger2.info('---------第{}轮训练开始---------'.format(epoch + 1))
    ...
    logger1.info(f'{acc_list}')
    logger1.info(f'{loss_list}')
