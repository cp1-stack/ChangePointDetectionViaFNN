...
train_dataset = CPDatasets('ChangePoints_data_train03.csv')
train_loader = DataLoader(dataset=train_dataset, batch_size=64, shuffle=True, num_workers=2)

test_dataset = CPDatasets('ChangePoints_data_test03.csv')
    ...
    for epoch in range(Epoch):
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader, 0):
                ...
                y_pred_label = torch.where(outputs >= 0.4, torch.tensor([0.5]), torch.tensor([0.0]))
